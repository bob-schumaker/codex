...
`controller/requestParticipation` is rate-limited by global unauthenticated-connection caps. It activates an out-of-band, connection-bound `ControllerEnrollmentGrant` for the TUI main thread; it does not ask the TUI for a prompt or require TUI input. A `ControllerEnrollmentGrant` records the trusted issuer or user ceremony, controller subject identity such as a public key, allowed main-thread scope, authorization epoch, expiry, and revocation source. The source of truth for this grant is not the per-launch socket, metadata file, or JSON-RPC session. It is a durable controller enrollment record owned by Codex user configuration or the platform credential store, with a revocation epoch that the launch reads at startup and rechecks on every participation, policy, and credential-change event. A launch may cache grant verification for its live `ConnectionId`, but the cached grant is invalidated by expiry, revocation epoch change, credential rotation, sign-off, disconnect, or main-thread close. During the authenticated Upgrade, the controller proves possession of that identity over the launch nonce and connection challenge; the runtime binds the resulting grant to the `ConnectionId`. Credential rotation, expiry, policy changes, and explicit user "forget controller" revocation enter the same serialized authorization-revocation path. Its params contain a requested controller name (for example, `codex-waveshare`) and human-readable description. Both are untrusted display claims, not an identity or authorization principal. Same-account socket access alone never satisfies the grant.

The request returns `approved` only after the runtime verifies the live connection's enrollment grant. If the main thread is `TuiOwned` and TUI-viable, approval also issues the initial lease and transfers ownership. If another controller owns it, approval creates a read-capable standing session with `activeLease: null`; it never displaces that controller, and the caller may later use `controller/acquireControl`. If the main thread does not yet exist, the request returns a retryable typed `main-thread-unavailable` error with `retryAfterMs` and `launchState: starting`; the controller may repeat `controller/requestParticipation` on the same initialized connection or reconnect after the hint. If main-thread creation fails, the immutable main thread closed, or the launch entered terminal `TuiUnavailable`, the request returns a non-retryable typed error such as `main-thread-closed` or `tui-unavailable`. Disconnect, sign-off, policy revocation, credential expiry, or main-thread close deactivates the grant. A rejected connection has no thread access and can only close, use protocol keepalives, or repeat enrollment through the external controller authorization flow.

The enrollment grant means “authorize this controller to be the input method” for the TUI main thread; it is not read-only observation consent or a one-shot handoff. It records an approved, connection-bound controller session with standing control-switch authorization; the session can be read-capable with no active lease or can own the main thread through an `interactive-control` lease. A server-monotonic lease deadline ends ownership and returns prompts to the TUI while preserving the session; a server-monotonic authorization deadline destroys the session and its read access. The wire response exposes only advisory `leaseExpiresInMs` and `authorizationExpiresInMs`, sampled with the response; client clocks never enforce them.

The controller session is bound to its live `ConnectionId`. A lease ID is an opaque audit and revocation identifier returned in controller-session status, not a reusable bearer credential: no existing app-server RPC gains a token field, and copying a lease ID to another connection grants nothing. The discovery nonce proves that a controller reached the intended endpoint; it does not authenticate or authorize the controller.
...
The controller uses the existing app-server mutation RPC shapes rather than a parallel control protocol. Before implementation, the generated v2 registry must enumerate every method, server-request response, implicit target, cursor, resume token, and subscription on two independent axes:
...
Enrollment, credential verification, policy revocation, and single-thread assignment are controller-authorization mechanisms, not public app-server RPCs. Before implementation, the public v2 types must use `*Params`/`*Response`/`*Notification`, camelCase serde and TypeScript names, `#[ts(export_to = "v2/")]`, and the normal experimental annotations/schema generation workflow.

This adds only a participation/control-session wrapper around the normal app-server interface. Once its `ControllerEnrollmentGrant` is active, the controller has standing authority to acquire or release input ownership for the granted main thread. It is the normal input client while it owns that thread; any thread-affecting TUI input automatically cancels that active input authorization and reclaims the thread, while TUI policy can explicitly revoke access at any time.
...
- Participation activation is rate-limited and TUI-independent: the runtime verifies a live connection-bound `ControllerEnrollmentGrant` against durable Codex user configuration or the platform credential store and returns `approved` or `rejected`. An asserted controller name never supplies identity or authority.
...
- A disconnected or reconnected controller cannot retain or recover a prior lease or standing authorization without a new controller enrollment grant.
